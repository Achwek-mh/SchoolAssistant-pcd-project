...
with jsonlines.open("professeur (1).jsonl") as f:
    data = list(f)
...
""" query = '''SELECT question, reponse FROM professeur 
            UNION
            SELECT question , reponse FROM result_exam
            Union
            SELECT question , reponse FROM result_pcd
            Union
            SELECT question , reponse FROM result_pe
            union
            SELECT question , reponse FROM result_pfe
            union
            SELECT question , reponse FROM sujet_pcd
            '''
cursor = db.cursor()
