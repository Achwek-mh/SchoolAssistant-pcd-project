...
const ResultPCD = () => {
 ...
 useEffect(() => {
  ...
  const handleSubmit = async(e) => {
    e.preventDefault();
     const pcd ={Description:currentUser.Description,Nom:currentUser.Nom,Encadrant:currentUser.Encadrant,Mots_clés:currentUser.Mots_clés,Domaine:currentUser.Domaine}
        sujetService.savepcd(pcd).then(res=>{ 
          if (res){
            console.log(res)
...
const update=async(e)=>{
  ...
  toggle() ;
  e.preventDefault();
  sujetService.updatesujetpcd(id,currentUser).then(res=>{ 
  if (res){
  Swal.fire({ 
...
const deletee =async(tdata,e)=>{
  ...
  e.preventDefault();
  resultService.deleteresultpcd(tdata.id).then(res=>{
    if(res){
  ...
  return (
    ...
    <Card className="card1" >
    <CardBody className="cardbody1"  /* style={{width:1500,marginLeft:'-50px'}} */ >
